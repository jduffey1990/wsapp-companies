...
const pool = new Pool({
    ...
    ssl: ((_a = process.env.DATABASE_URL) === null || _a === void 0 ? void 0 : _a.includes('neon.tech'))
...
function generateRetailer() {
    ...
    const state = randomFromArray(STATES[region]);
    const citiesForState = CITIES[state] || [`City${randomInt(1, 100)}`];
    ...
    const carriedCategories = Array.from({ length: numCategories }, () => randomFromArray(CATEGORIES)).filter((v, i, a) => a.indexOf(v) === i); // Unique
    // Target demographic
    ...
    const targetAgeGroup = Array.from({ length: numAgeGroups }, () => randomFromArray(ageGroups)).filter((v, i, a) => a.indexOf(v) === i);
    // Seasonality
    ...
    const paymentTerms = Array.from({ length: numTerms }, () => randomFromArray(PAYMENT_TERMS)).filter((v, i, a) => a.indexOf(v) === i);
    return {
...
function seedRetailers() {
    return __awaiter(this, arguments, void 0, function* (count = 100) {
        console.log(`🌱 Seeding ${count} mock retailers...`);
        const client = yield pool.connect();
        try {
            yield client.query('BEGIN');
            // Check if retailers already exist
            const checkResult = yield client.query('SELECT COUNT(*) FROM retailers');
            const existingCount = parseInt(checkResult.rows[0].count);
            if (existingCount > 0) {
                console.log(`⚠️  Found ${existingCount} existing retailers. Clearing table...`);
                yield client.query('DELETE FROM retailers');
            }
